...
export abstract class ReportAbstract implements ReportModel {
  ...
  initCriteria(criteria: ReportCriteriaDto, typeSearch: string) {
    // Ne transmettre que les données nécessaires selon le niveau hiérarchique
    switch (typeSearch) {
      case 'CODEBOITE':
        // Pour CODEBOITE -> charger les APPs
        criteria.codeBoite = this.report?.replication?.boite ?? ''
        delete criteria.app
        delete criteria.db
        delete criteria.schema
        delete criteria.support
        delete criteria.typeSupport
        break
      case 'APP':
        // Pour APP -> charger les DBs
        criteria.codeBoite = this.report?.replication?.boite ?? ''
      ...
      case 'DB':
        ...
        break
      case 'SCHEMA':
        // Pour SCHEMA -> charger les REPLICATIONs (tous les critères)
        criteria.codeBoite = this.report?.replication?.boite ?? ''
        criteria.app = this.report?.replication?.app ?? ''
        criteria.db = this.report?.replication?.database ?? ''
  ...
  async initData(resolve: (data: ReportModel[]) => void, typeSearch: string) {
    console.log('initData typeSearch : ', typeSearch)
    this.initCriteria(this.criteria, typeSearch)
    
    try {
      // Utilisation de l'API REST au lieu de l'API Mock
      const reports = await api.reportApi.findReports(this.criteria)
...
export class ReportSchema extends ReportAbstract {
  constructor(report: NodeReportDto, criteria: ReportCriteriaDto, key: number) {
    super(report, criteria)
    this.key = key
    this.id = `${this.report.replication?.boite}/${this.report.replication?.app}/${this.report.replication?.database}/${this.report.replication?.supportType}/${this.report.replication?.schema}`
